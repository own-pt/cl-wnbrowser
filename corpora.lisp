...
(defun load-verbnet ()
  ...
  (fare-csv:with-rfc4180-csv-syntax ()
    (dolist (row (fare-csv:read-csv-file "corpora/verbnet.br.csv"))
      (let ((pt (first row))
	    (en (third row)))
...
(defun load-verbocean ()
  ...
  (fare-csv:with-rfc4180-csv-syntax ()
    (dolist (row (fare-csv:read-csv-file "corpora/verbocean-translated.csv"))
      (let ((en (first row))
	    (pt (second row)))
...
(defun load-portal-da-lingua-portuguesa ()
  ...
  (with-open-file (stream "corpora/portal-da-lingua-pt.txt")

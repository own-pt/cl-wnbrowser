...
(defun load-verbnet ()
  ...
  (fare-csv:with-rfc4180-csv-syntax ()
    (dolist (row (fare-csv:read-csv-file (merge-pathnames "corpora/verbnet.br.csv" *basedir*)))
      (let ((pt (first row))
	    (en (third row)))
...
(defun load-verbocean ()
  ...
  (fare-csv:with-rfc4180-csv-syntax ()
    (dolist (row (fare-csv:read-csv-file (merge-pathnames "corpora/verbocean-translated.csv" *basedir*)))
      (let ((en (first row))
	    (pt (second row)))
...
(defun load-portal-da-lingua-portuguesa ()
  ...
  (with-open-file (stream (merge-pathnames "corpora/portal-da-lingua-pt.txt" *basedir*))
